        ...
        try:
            for line in env_file.read_text(encoding="utf-8").splitlines():
                ...
                if line.startswith("#") or "=" not in line:
                    continue
                k, v = line.split("=", 1)
                if k.strip().upper() in ("GEMINI_API_KEY", "GOOGLE_API_KEY"):
...
def get_gemini_key() -> str | None:
    stored = load_api_keys().get("gemini_api_key")
    if stored and len(str(stored).strip()) > 5:
...
def get_assistant_name() -> str:
    """Return the configured assistant name, or 'AUREX' if not set."""
    return load_api_keys().get("assistant_name", "AUREX") or "AUREX"
...
def save_assistant_config(assistant_name: str, user_name: str) -> None:
    ...
    if CONFIG_FILE.exists():
        try:
            ...
            data = {}
    data["assistant_name"] = assistant_name.strip() or "AUREX"
