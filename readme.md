# ⚡ AUREX MARK LIV
### The Ultimate Cross-Platform Personal AI Assistant — Made by Hammad Nawaz
...
| 🎙️ Wake Word | Local **"Hey Aurex"** detection — sleeps until called, auto-sleeps after 2 min of silence, and never streams audio while asleep |
...
| 👁️ Memory Panel | See every fact AUREX has stored about you, when it learned it, and delete any of it in one click |
...
| 🧩 Plugin System | Drop a single `.py` file into `plugins/` — AUREX learns a new skill on next launch |
| 🎙️ Real-time Voice | Ultra-low latency conversation in any language via Gemini Live API |
| 🎨 Live Theming | Recolour the entire HUD from a hue wheel or hex — the avatar retints with it |
| 〰️ Reactive HUD | Waveform pulses to real audio — your mic while listening, AUREX while speaking |
...
Nothing on the core moves for decoration. The rings speed up when AUREX is thinking, the spikes are the actual waveform, and the colour is the state — the same language the face speaks, without the face.

#### 🗣️ It answers before it works
Some replies used to open with three or four seconds of silence: not because a tool was slow, but because AUREX was still *writing the tool call* — a set of quiz questions, the findings from a contract. The tool was instant; the composing was not, and from the user's side those are the same thing.
...
* Asking AUREX to look at the screen produced **two different answers** — the flow made it speak once *before* the image arrived, so it improvised, and again after. The frame is now attached to the same exchange as its tool result: one turn, one answer, one fewer round trip.
...
AUREX moves files, renames them, writes to them and changes your settings. None of that had a way back; if it misheard you, the only remedy was to fix it by hand.
...
The token is now issued by the interface. Shutdown, restart and WiFi put a banner on the HUD and **return immediately**; the action runs only if you press CONFIRM. Nothing blocks — AUREX keeps talking while the banner is up — so this is **cheaper than the old gate**, which burned two tool round trips on every power command.
...
Both audio streams opened with no device argument at all, so they always took whatever the OS called "default" — and on Windows that *moves on its own* the moment you plug a headset in. "AUREX can't hear me" almost always meant "AUREX is listening to the webcam".
...
```bash
git clone https://github.com/Hammad Nawaz/Mark-LIV.git
cd Mark-LIV
...
| **Microphone** | Required for voice interaction (and for the "Hey Aurex" wake word) |
...
├── plugins/
│   ├── quiz.py               # Interactive quiz — AUREX writes the questions, you answer on screen
...
│   └── wake_word.py          # Local "Hey Aurex" detector — own thread, offline, opt-in
└── config/
...
**Made by Hammad Nawaz**

Engineered by Hammad Nawaz building the next generation autonomous AI assistant.
